...
mod platform {
    ...
    fn build_menu(app: &AppHandle, relay: &RelayRuntime) -> Result<tauri::menu::Menu<tauri::Wry>> {
        ...
        let open_item = MenuItemBuilder::with_id(
            "open-settings",
            i18n::tray_open_settings(language),
        )
        .build(app)?;
...
mod platform {
    ...
    pub fn setup(_app: &AppHandle, _relay: RelayRuntime) -> Result<()> {
        Ok(())
    }

    pub fn refresh(_app: &AppHandle, _relay: &RelayRuntime) -> Result<()> {
        Ok(())
    }
}

pub use platform::{refresh, setup};
